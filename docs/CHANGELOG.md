...
- **K1 upgraded to capability token (all four reviewers)**: After Gemini's K1 review confirmed the flag pattern was honor-system one level up (unanimous: Claude Chat, Grok, Kimi, Gemini), Option A was replaced with the capability token pattern. `DiversifyExitToken` (private constructor + `friend class CoherenceController`, move-only) added to `types.h`. `CoherenceController::check_diversify_complete()` now returns `DiversifyExitToken` — the only legal token source. `CharterFSM::to_structuring(DiversifyExitToken, reason)` consumes the token; a caller cannot fabricate one without going through `check_diversify_complete()`. `declare_diversify_complete()` and `diversify_complete_declared_` flag removed entirely. `DiversifyCompletionResult` struct removed; unmet criteria are accessible via `token.unmet_criteria()`. See `OPEN_QUESTIONS.md` K1 (CLOSED).
